...
def convert_level_to_severity(level):
    """
    Convert Wazuh level (0-15) to TheHive severity (1-4)
...
def generate_alert(format_alt, artifacts_dict, w_alert):
    ...
    # Get Wazuh level and convert to TheHive severity
    wazuh_level = int(w_alert['rule']['level'])
    severity = convert_level_to_severity(wazuh_level)
    logger.info(f'Converting Wazuh level {wazuh_level} to TheHive severity {severity}')
...
if __name__ == "__main__":
    try:
        ...
        logger.exception(f'Error in script: {str(e)}')
